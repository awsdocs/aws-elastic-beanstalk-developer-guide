...
Your environment's Auto Scaling Group can manage a [Spot Fleet](#environments-cfg-autoscaling-spot) for your environment\. These combine On\-Demand and Spot Instances\.
...
**Topics**
+ [Spot Fleet Support](#environments-cfg-autoscaling-spot)
...
## Spot Fleet Support<a name="environments-cfg-autoscaling-spot"></a>

To take advantage of Amazon EC2 [Spot Instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-spot-instances.html), you can enable a *Spot Fleet* for your environment\. A Spot Fleet combines Amazon EC2 purchase options and maintains a mix of On\-Demand and Spot Instances\. When you enable Spot Fleet requests, your environment's Auto Scaling Group manages the environment's Spot Fleet\.
...
If you use rolling updates, be sure that the maximum instance count is higher than the [**Minimum instances in service** setting](using-features.rollingupdates.md#rollingupdates-configure) for rolling updates\.
   + **Fleet composition** – Select **Combined purchase options and instance types** to enable Spot Fleet requests\.
...
Options related to enabling Spot Fleet requests and configuring the Spot Fleet\.

The following example creates an environment and configures the Auto Scaling Group to enable Spot Fleet requests for the new environment, with three possible instance types to use\.
...
The [`aws:ec2:instances`](command-options-general.md#command-options-general-ec2instances) namespace provides options related to your environment's instances, including Spot Fleet management\. It complements [`aws:autoscaling:launchconfiguration`](command-options-general.md#command-options-general-autoscalinglaunchconfiguration) and [`aws:autoscaling:asg`](command-options-general.md#command-options-general-autoscalingasg)\.

The following [configuration file](ebextensions.md) example configures the Auto Scaling Group to enable Spot Fleet requests for your environment, with three possible instance types to use, at least one On\-Demand Instance as baseline capacity, and a sustained 33% of On\-Demand Instances for any additional capacity in the Spot Fleet\.
